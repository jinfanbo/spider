...
class FlyscootSpider(object):
    ...
    def parseHtml(self, html):
        ...
        for data in dates:
            ...
            # 起飞时间
            arr_time = re.match('.* (.*):(.*)', data.xpath('//div[@class="flight__from"]//li[@class="flight-time"]/text()')[i])
            ...
            # 航班型号为AK
            dct["carrier"] = re.match('.*?: (.*?) ', data.xpath('//div[@class="flight__stop"]//div[@class="time"]/@data-content')[i]).group(1)
            # 落地城市为KUL
            dct["depAirport"] = data.xpath('//div[@class="flight-results__result"]/@data-arrstation')[i]
            # 落地时间
            #Nov 02, 2018 10:05
            dep_time = re.match('(.*?) (.*), (.*) (.*):(.*)', data.xpath('//div[@class="flight-results__result"]/@data-time')[i])
            ...
            # 航班编号
            dct["flightNumber"] = re.match('.*?: (.*?) \\(', data.xpath('//div[@class="flight__stop"]//div[@class="time"]/@data-content')[i]).group(1).replace(' ','')
